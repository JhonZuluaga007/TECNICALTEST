...
| 9 | Dedupe, reusable components, CI and coverage | ✅ Done |
...
### Phase 9 — What belongs in `core/`, and a gate that runs itself

The design system's foundations were already sound and this phase does not touch them: no `Colors.*` or `Color(0x` outside `core/design_system/`, no `TextStyle(` or `fontSize:` anywhere in `lib/`, no hardcoded user-facing copy, `AppSpacing` on every `EdgeInsets`. What was missing was the layer above them — `AppTheme` configured **no component sub-theme at all** — and a criterion for `core/`'s front door.

**One rule sorted four widgets: `core/` is for what more than one feature uses.** `CardCatWidget` moved *down* to `features/landing_cats/` as `BreedCard` — both call sites were there, and it reached for `l10n.moreAction` and `l10n.intelligenceLabel` itself, so it could only ever render that one card. The status views moved *up* to `core/`, where the detail screen had already been importing them across the feature boundary. `ThemeModeButton` moved out of `landing_page.dart` into the settings feature whose cubit it drives. And `NetworkImageWidget` was **deleted**.

**Genericizing the card was rejected, and its reusable half moved the other way.** Adding `actionLabel`/`characteristicLabel`/`characteristicValue` would have bought an abstraction with one implementation. What *is* shared about a card is its chrome — elevation, corner radius, outline — and in Flutter the reusable form of that is `ThemeData.cardTheme`, not a widget. Composition went down into the feature, chrome went up into the theme, and the app has more card reuse than before: any `Card` any feature writes now inherits it with no import.

**A promise four documents made turned out to be impossible.** `CLAUDE.md`, two source comments and the Phase 8 PR doc all said Phase 9 would promote `BreedImage` to `core/`. It imports `GetBreedImageUseCase` and `BreedImageCubit`, both landing-owned, so the promotion would invert the one-way dependency rule. It stays where it is, permanently, and that is now recorded as a decision instead of sitting on a list as work someone keeps trying to do.

**A real defect, found while auditing rather than while debugging.** `network_image.dart`'s `errorBuilder` branched on `stackTrace != null` and, when null, returned `Image.network(imageUrl)` — re-issuing the request that had just failed, from inside its own error handler, with no `errorBuilder` on the retry. The fix was the deletion: `BreedImage` absorbed the widget and both failure causes now land on one `_fallback()`. Verified by pasting the original back and watching two tests go red.

**Tokens, drawn at a measured line.** A value earns a token when the same number expresses one decision at more than one *independent* site. So `AppRadius.sm`/`.md` for the two radii, `elevation: 2` as a plain number inside `cardTheme`, and **`width: 1` deleted outright** — it is the SDK default for both `BorderSide` and `Border.all`. Icon sizes stayed literals, and the trap is instructive: the `48` on a status icon and the `48` on the app bar's bottom are not the same decision, so a token would have fused two unrelated numbers.

**CI, in two jobs, with the gap closed on purpose.** Linux runs format, analyze, test, the coverage ratchet and the stale-codegen check Phase 5 asked for by name. macOS runs the goldens, on the platform that generated them. A `golden` tag declared in `dart_test.yaml` is what one job excludes and the other selects — **the same tag from opposite sides**, so no file can fall between them and read as passing because nothing ran it. Verified: 327 + 4 = 331.

**Lints measured instead of assumed.** `unawaited_futures` is in, and its first catch was not a forgotten `await` — `_inFlight` is a `Map<String, Future<...>>`, so `remove()` returns a `Future` that the `finally` discards. It now says `unawaited(...)`. Four other candidate rules measured **0 hits** and are not enabled. `avoid_redundant_argument_values` measured 22 hits, 20 of them test fixtures where an explicit value is documentation, and both `lib/` hits deliberate — dropped, with the numbers written into `analysis_options.yaml` rather than silently omitted.

**Honest notes.** A deliberate mutation — emitting the status views' action gap unconditionally, which pushes the empty view 16 px off centre — left all 328 tests green. Nothing covered it; the three cases that do now exist because the mutation asked for them, not because they were designed. In `breed_image_test.dart`, one case passes with the bug and without it: it exercises the `ImageUnavailable` branch, so it is a guard, not evidence. `CatsEmptyView` and `StaleBanner` were promoted on their siblings' evidence rather than their own, since each still has exactly one consumer. The `sed` that renamed `CardCatWidget` rewrote a comment that deliberately named the *old* class — in a repo where comments are load-bearing, a global identifier rename is not a safe mechanical operation. The import-depth break the plan predicted still shipped into the working tree and made the suite report 313 instead of 318, five tests silently not loading; verification caught it, the author did not. And coverage rising 95.5% → 95.8% is mostly `network_image.dart` being deleted — removing untested code raises a percentage without testing anything. The real news is that the theme toggle and the rating meter had **no test files at all** and now have ten cases between them.

**Still unverified:** whether the goldens reproduce on a second host. Pinning the job to macOS sidesteps that question rather than answering it.
